...
export function isImageGenerationModel(
  ...
  if (
    ...
    hostMatches(provider.baseUrl || '', 'googleapis.com')
  )
    return false
  return (
...
export function inferImageStudioProtocol(
...
function usesAsyncImageGateway(base: string, model: string): boolean {
  ...
  const imagesApiModel =
    model.includes('gpt-image') || model.startsWith('dall-e')
...
export function reconcileImageStudioProtocol<
  T extends { protocol: string; model: string },
>(config: T, provider?: ModelProvider): T {
  const inferred = inferImageStudioProtocol(provider, config.model)
  if (
    provider &&
    config.protocol === 'async' &&
    inferred === 'openai' &&
  ...
  if (
    inferred === 'grok' ||
    inferred === 'gemini' ||
    inferred === 'gemini-chat'
  ) {
    return config.protocol === inferred
      ? config
      : { ...config, protocol: inferred }
  }
