...
export function ChatRouteKeepAlive({ activeKey, children }: ChatRouteKeepAliveProps) {
  ...
  const keep = activeKey === 'conversation' || activeKey === 'settings' || activeKey === 'images' || activeKey === 'videos'
      ...
      {[...cacheRef.current.entries()].map(([key, node]) => {
        ...
        return (
          <div
            ...
            {...(active ? {} : { inert: '' })}
          >
            <RouteActiveContext.Provider value={active}>{node}</RouteActiveContext.Provider>
          </div>
        )
      })}
