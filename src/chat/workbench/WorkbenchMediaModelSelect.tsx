...
export function WorkbenchMediaModelSelect({ kind, children }: { kind: MediaPoolKind; children?: ReactNode }) {
  const zh = useLang() === 'zh'
  ...
  const workflow = provider?.request.comfy?.workflows.find(w => w.id === selected?.model)
  return <><div className="workbench-field">
    <span>{title}</span>
    ...
    {provider && workflow ? <ComfyWorkflowRunner key={`${provider.id}:${workflow.id}`} provider={provider} workflow={workflow} /> : children}
  </>
}
