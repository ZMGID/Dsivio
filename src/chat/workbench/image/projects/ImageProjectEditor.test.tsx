...
function bootstrap(tasks: ImageTask[] = []): ImageBootstrap {
  testFeature = tasks[0]?.brief.feature || 'gen'
  return {
    tasks,
    config,
...
beforeEach(() => {
  ...
  vi.mocked(api.workbenchImageImport).mockResolvedValue([importedProduct()])
})
...
describe('Image workspace state gates', () => {
  it('switches tasks without saving remotely and restores each task local edits', async () => {
    ...
    vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(bootstrap([current, target]))
    vi.mocked(api.workbenchImageGet).mockImplementation(async id => id === target.id ? target : current)
    vi.mocked(api.workbenchImageSavePlans).mockRejectedValue(new Error('保存失败也不能阻塞切换'))
    render(<ImageStudio />)
    fireEvent.click(await screen.findByRole('button', { name: '记录' }))
    expect(api.workbenchImageSavePlans).not.toHaveBeenCalled()
    fireEvent.click(await screen.findByRole('button', { name: '打开任务 另一个任务' }))
    await waitFor(() => expect(api.workbenchImageGet).toHaveBeenCalledWith(target.id))
  ...
  it('allows the empty result page and keeps separate drafts across image features', async () => {
    render(<ImageStudio />)
    await waitFor(() => expect(api.workbenchImageBootstrap).toHaveBeenCalled())
    ...
    cleanup(); render(<ImageStudio feature="workflow" />)
    fireEvent.change(screen.getByLabelText('制作要求'), { target: { value: '保留模板制作草稿' } })
    cleanup(); render(<ImageStudio feature="gen" />)
  ...
  it('can start a new draft during a pending save without the old response taking it over', async () => {
    ...
    vi.mocked(api.workbenchImageSave).mockImplementation(() => new Promise(resolve => { finish = resolve }))
    vi.mocked(api.workbenchImageAction).mockResolvedValue({ ...saved, status: 'running' })
    render(<ImageStudio />)
    await waitFor(() => expect(api.workbenchImageBootstrap).toHaveBeenCalled())
  ...
  it('lets edits survive polling a running image task', async () => {
    ...
    vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(bootstrap([running]))
    vi.mocked(api.workbenchImageGet).mockResolvedValue({ ...running, status: 'completed' })
    render(<ImageStudio />)
    ...
    await waitFor(() => expect(api.workbenchImageGet).toHaveBeenCalledWith(running.id), { timeout: 2000 })
...
describe('Built-in image workflows', () => {
  it('starts a text-only image directly, with no preparation controls or mandatory plan step', async () => {
    ...
    expect(screen.getByRole('button', { name: '分辨率' })).toHaveTextContent('1K')
    ...
    expect(api.workbenchImageSave).toHaveBeenCalledWith(expect.objectContaining({ language: 'auto', count: 0, products: [] }), undefined, undefined, config)
  ...
  it('keeps replacement examples separate from product photos and accepts no extra prompt', async () => {
    ...
    vi.mocked(api.workbenchImageImport).mockResolvedValueOnce([{ ...importedProduct(), assets: [sample] }])
    vi.mocked(api.workbenchImageSave).mockImplementation(async (brief) => ({ ...fixture(), brief }))
    vi.mocked(api.workbenchImageAction).mockResolvedValue({ ...fixture(), status: 'running' })
    render(<ImageStudio />)
    cleanup(); render(<ImageStudio feature="replace" />)
    ...
    await waitFor(() =>     expect(api.workbenchImageSave).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('searches templates beside the product dropzone and keeps matching heights', async () => {
    ...
    vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(data)
    render(<ImageStudio />)
    cleanup(); render(<ImageStudio feature="smart" />)
  ...
  it('refreshes shared chat templates on focus without replacing the unsaved image brief', async () => {
    ...
    vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(next)
    fireEvent(window, new Event('focus'))
    await waitFor(() => expect(api.workbenchImageBootstrap).toHaveBeenCalledTimes(2))
    expect(screen.getByLabelText('图片要求')).toHaveValue('尚未保存的需求')
    const saved = fixture()
    saved.brief = { ...emptyBrief('gen'), requirement: '尚未保存的需求' }
    vi.mocked(api.workbenchImageSave).mockResolvedValue(saved)
    cleanup(); render(<ImageStudio feature="smart" />)
    expect(await screen.findByRole('option', { name: /聊天新建模板/ })).toBeInTheDocument()
  })
  it('uses the shared theme, scrollbar, input and select controls', async () => {
    ...
    expect(screen.queryByRole('button', { name: '使用平台' })).not.toBeInTheDocument()
    const ratio = screen.getByRole('button', { name: '比例' })
    expect(ratio).toHaveClass('kv-select')
    fireEvent.click(ratio)
    expect(await screen.findByRole('listbox')).toHaveClass('kv-select-menu', 'custom-scrollbar')
    fireEvent.click(screen.getByRole('option', { name: '1:1 正方形' }))
    expect(screen.getByRole('button', { name: '比例' })).toHaveTextContent('1:1')
    expect(screen.queryByLabelText('任务名称')).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: '模型与保存位置' }))
    expect(screen.getByRole('dialog', { name: '模型与保存位置' })).toHaveClass('kv-modal', 'custom-scrollbar')
  })
  it('opens saved tasks from a dedicated page instead of the rail', async () => {
    ...
    await waitFor(() => expect(api.workbenchImageGet).toHaveBeenCalledWith('job'))
  ...
  it('saves structured inputs before invoking a dedicated image Agent action', async () => {
    ...
    vi.mocked(api.workbenchImageSave).mockResolvedValue(t)
    vi.mocked(api.workbenchImageAction).mockResolvedValue({ ...t, progress: '规划完成' })
    render(<ImageStudio />)
    await waitFor(() => expect(screen.getByRole('button', { name: '生成图片' })).toBeDisabled())
    fireEvent.change(screen.getByLabelText('图片要求'), { target: { value: '纯白底，保留产品细节' } })
    fireEvent.click(screen.getByRole('button', { name: '生成图片' }))
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith('job', 2, { kind: 'start', group: '未分类' }, config),
    )
  ...
  it('records human approval as a separate action, then offers bulk generation', async () => {
    ...
    vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(bootstrap([t]))
    vi.mocked(api.workbenchImageGet).mockResolvedValue(t)
    vi.mocked(api.workbenchImageAction).mockResolvedValue({ ...t, approvedGroups: ['背包'] })
    render(<ImageStudio />)
    await openSavedTask('背包秋季套图')
    fireEvent.click(await screen.findByRole('button', { name: /确认样品效果/ }))
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith('job', 2, { kind: 'approve', group: '背包' }, config),
    )
  ...
  it('resumes a persisted remote job instead of submitting a second generation', async () => {
    ...
    vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(bootstrap([t]))
    vi.mocked(api.workbenchImageGet).mockResolvedValue(t)
    vi.mocked(api.workbenchImageAction).mockResolvedValue(t)
    render(<ImageStudio />)
    await openSavedTask('背包秋季套图')
    fireEvent.click(await screen.findByRole('button', { name: '恢复查询' }))
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith('job', 2, {
        kind: 'resume',
        resultId: 'a-2',
        group: '背包',
      }, config),
...
describe('Replace sample drag-drop', () => {
  it('imports images dropped on the sample panel as layout sources', async () => {
    const sample = { id: 'layout', path: 'assets/layout.png', name: '样图.png' }
    vi.mocked(api.workbenchImageImport).mockResolvedValue([{ ...importedProduct(), assets: [sample] }])
    render(<ImageStudio />)
    cleanup(); render(<ImageStudio feature="replace" />)
    ...
    await waitFor(() => expect(api.workbenchImageImport).toHaveBeenCalledWith(['C:/layout.png'], false))
...
describe('Product material drag-drop', () => {
  it('imports dropped images as reference photos', async () => {
    ...
    await waitFor(() =>
      expect(api.workbenchImageImport).toHaveBeenCalledWith(['C:\\goods\\front.png'], false),
    )
    fireEvent.click(screen.getByText('更多设置'))
  ...
  it('imports dropped folders as SKUs', async () => {
    vi.mocked(api.workbenchImageImport).mockResolvedValue([importedProduct('sku-a')])
    render(<ImageStudio />)
    await waitFor(() => expect(dropHandler).toBeTypeOf('function'))
    dropHandler?.({
    ...
    await waitFor(() =>
      expect(api.workbenchImageImport).toHaveBeenCalledWith(
...
it('keeps image work alive through chat navigation and does not replace a new brief', async () => {
  ...
  vi.mocked(api.workbenchImageSave).mockResolvedValue(saved)
  vi.mocked(api.workbenchImageAction).mockReturnValue(pending)
  ...
  fireEvent.change(screen.getByLabelText('图片要求'), { target: { value: saved.brief.requirement } })
  fireEvent.click(screen.getByRole('button', { name: '生成图片' }))
  await waitFor(() => expect(api.workbenchImageAction).toHaveBeenCalled())
  fireEvent.click(screen.getByRole('button', { name: '记录' }))
  expect(screen.getByRole('heading', { name: /图片任务/ })).toBeInTheDocument()
...
it('keeps unsaved changes when an existing task changes during a focus refresh', async () => {
  ...
  vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(bootstrap([task]))
  render(<ImageStudio />)
  ...
  vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(bootstrap([{ ...task, revision: task.revision + 1, progress: 'remote change' }]))
...
it('keeps image task controls stable during reads and ignores abandoned destinations', async () => {
  ...
  vi.mocked(api.workbenchImageBootstrap).mockResolvedValue(bootstrap([target]))
  let finish!: (value: ImageTask) => void
  vi.mocked(api.workbenchImageGet).mockReturnValueOnce(new Promise(resolve => { finish = resolve }))
  render(<ImageStudio />)
  ...
  expect(screen.getByRole('button', { name: '模型与保存位置' })).toBeEnabled()
  fireEvent.click(screen.getByRole('button', { name: '新建图片' }))
