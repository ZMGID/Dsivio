...
import ImageStudio from './ImageProjectWorkspace'
import {
  emptyBrief,
...
beforeEach(() => {
  ...
  vi.mocked(api.workbenchImagePreview).mockResolvedValue('data:image/png;base64,aW1hZ2U=')
})
...
describe('制作、试品、反馈和持续出图', () => {
  it('preserves the existing feature entries and exports to the task folder without another picker', async () => {
    ...
    load(initial)
    vi.mocked(api.workbenchImageExport).mockResolvedValue('C:/Images/backpack/deliveries/batch')
    render(<ImageStudio feature="workflow" />)
    ...
    await waitFor(() => expect(api.workbenchImageExport).toHaveBeenCalledWith('workflow', '', 800, 800, 2048))
  ...
  it('accepts source material separately from trial products and starts rule creation in one action', async () => {
    ...
    vi.mocked(api.workbenchImageImport).mockResolvedValue([{ ...product('source'), assets: [source] }])
    vi.mocked(api.workbenchImageSave).mockImplementation(async (brief: ImageBrief) => ({
      ...made,
      brief,
    }))
    vi.mocked(api.workbenchImageAction).mockResolvedValue(made)
    render(<ImageStudio feature="workflow" />)
    ...
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith('workflow', 2, {
        kind: 'workflow_build',
        group: '未分类',
      }, config),
    )
  ...
  it('restores a completed trial, confirms it and saves appended products into the same workflow', async () => {
    ...
    vi.mocked(api.workbenchImageAction).mockResolvedValue(approved)
    render(<ImageStudio feature="workflow" />)
    fireEvent.click(await screen.findByRole('button', { name: '试品满意，确认这版' }))
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith(
        'workflow',
        2,
        expect.objectContaining({ kind: 'workflow_approve' }), config
      ),
    )
    ...
    vi.mocked(api.workbenchImageImport).mockResolvedValue([product('b')])
    ...
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith(
        'workflow',
        3,
        expect.objectContaining({ kind: 'workflow_produce' }), config
      ),
    )
    expect(api.workbenchImageSave).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(api.workbenchImageImport).toHaveBeenCalledWith(['C:/b-front.png', 'C:/b-back.png'], false)
  })
  ...
  it('sends shared feedback with trial context and blocks approval until the new version has trial results', async () => {
    ...
    next.plans = []
    vi.mocked(api.workbenchImageAction).mockResolvedValue(next)
    render(<ImageStudio feature="workflow" />)
    ...
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith(
        'workflow',
        2,
        expect.objectContaining({ kind: 'workflow_refine', note: '产品放大，后续都用这个比例' }), config
      ),
    )
  ...
  it('submits selected products for trial and requires all selected pages before approval', async () => {
    ...
    load(initial)
    vi.mocked(api.workbenchImageAction).mockResolvedValue({
      ...initial,
    ...
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith(
        'workflow',
        2,
        expect.objectContaining({ kind: 'workflow_trial', sampleIds: ['a', 'b'] }), config
      ),
    )
    expect(screen.getByRole('button', { name: '试品满意，确认这版' })).toBeDisabled()
  })
  ...
  it('edits common rules without rebinding assets and keeps single-image editing separate', async () => {
    const initial = task()
    load(initial)
    vi.mocked(api.workbenchImageAction).mockResolvedValue(initial)
    render(<ImageStudio feature="workflow" />)
  ...
  it('lets the user regenerate a remote result without recovering it first', async () => {
    ...
    await waitFor(() => expect(api.workbenchImageAction).toHaveBeenCalledWith('workflow', 2,
      expect.objectContaining({ kind: 'retry', productId: 'a', slotId: 'h1' }), config))
  })
  ...
  it('keeps recovery available without blocking shared refinement', async () => {
    ...
    await waitFor(() =>
      expect(api.workbenchImageAction).toHaveBeenCalledWith(
        'workflow',
        2,
        expect.objectContaining({ kind: 'resume', resultId: 'result-a' }), config
      ),
    )
  ...
  it('imports dropped files as original references instead of trial products', async () => {
    ...
    await waitFor(() =>
      expect(api.workbenchImageImport).toHaveBeenCalledWith(['C:\\source.png'], false),
    )
  ...
  it('imports files dropped on the trial panel as products, not original references', async () => {
    ...
    await waitFor(() =>
      expect(api.workbenchImageImport).toHaveBeenCalledWith(['C:\\goods\\b.png'], false),
    )
