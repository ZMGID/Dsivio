...
describe('studioModels', () => {
  it('repairs the stale async inference for a synchronous GPT image relay', () => {
    ...
    expect(
      reconcileImageStudioProtocol(
        { protocol: 'async', model: 'gpt-image-2' },
        provider,
      ).protocol,
    ).toBe('openai')
    expect(
      inferImageStudioProtocol(
        makeProvider({ baseUrl: 'https://relay.example/v1' }),
        'gpt-image-1',
      ),
    ).toBe('openai')
  })
  ...
  it('keeps catalog and adapter image models', () => {
    ...
    const google = makeProvider({
      ...
      enabledModels: ['imagen-4.0-generate-001', 'gemini-3.1-flash-image'],
    })
    expect(isImageGenerationModel(google, 'imagen-4.0-generate-001')).toBe(
      false,
    )
  ...
  it('infers studio protocol from the provider adapter instead of a user picker', () => {
    ...
    expect(
      inferImageStudioProtocol(
        makeProvider({ apiFormat: 'openai_chat' }),
        'gemini-3.1-flash-image',
      ),
    ).toBe('gemini-chat')
    expect(
    ...
    expect(inferImageStudioProtocol(makeProvider(), 'gpt-image-1')).toBe(
      'openai',
    )
    expect(
      inferImageStudioProtocol(
        makeProvider({ baseUrl: 'https://api.apimart.ai/v1' }),
        'gpt-image-1',
      ),
    ).toBe('async')
    expect(
  ...
  it('upgrades a stale openai protocol for relay gpt-image models', () => {
    ...
    expect(
      reconcileImageStudioProtocol(
        { protocol: 'async', model: 'custom-image' },
        relay,
      ).protocol,
    ).toBe('async')
    expect(
