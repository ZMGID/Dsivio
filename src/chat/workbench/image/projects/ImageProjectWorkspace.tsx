...
import type { ImageFormProps } from './ImageFormFields'
...
type View = ImageFeature | 'tasks'
type Stage = 'brief' | 'plan' | 'results'

export default function ImageProjectWorkspace({ feature = 'gen', Form = FreeImageForm }: { feature?: ImageFeature; Form?: ComponentType<ImageFormProps> }) {
  const [view, setView] = useState<View>(feature)
  const [stage, setStage] = useState<Stage>(() => readStudioDraft(`feature:${feature}`)?.stage || 'brief')
  const [brief, setBrief] = useState<ImageBrief>(
    () => readStudioDraft(`feature:${feature}`)?.brief || newImageDraftBrief(feature),
  )
  ...
  const [config, setConfig] = useState<ImageConfig>(() => {
    try { return JSON.parse(localStorage.getItem('dsivio.workbench.image-project-settings') || 'null') || DEFAULT_CONFIG } catch { return DEFAULT_CONFIG }
  })
  ...
  const shared = useSharedDraft('image', feature, native && !loading,
    ...
    async (draft) => {
      ...
      const saved = draft.taskId ? await api.workbenchImageGet(draft.taskId) : null
  ...
  useEffect(() => {
    ...
    void api
      .workbenchImageBootstrap()
      .then((data) => {
        if (alive) {
          setTasks(data.tasks.filter(task => task.brief.feature === feature))
          setTemplates(data.templates)
          // Read the old selection once; subsequent project polling must not overwrite this editor.
          setConfig(current => current.providerId ? current : { ...DEFAULT_CONFIG, ...data.config })
          setProviders(data.providers)
          const draft = readStudioDraft(`feature:${feature}`)
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      inFlight = true
      try {
        const data = await api.workbenchImageBootstrap()
        if (alive) {
          setTemplates(data.templates)
          setTasks(data.tasks.filter(task => task.brief.feature === feature))
          // Model selection belongs to this Workbench editor, not the polled project list.
          setProviders(data.providers)
  ...
  }, [native, loading, adopt, feature])
  useEffect(() => {
    setView((current) =>
      current === 'tasks' ? current : brief.feature,
    )
  }, [brief.feature])
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const current = await api.workbenchImageGet(taskId)
        if (cancelled) return
        ...
        if (current.status === 'running') timer = setTimeout(() => void poll(), 1200)
          ...
          const templates = await api.imageTemplatesList()
  ...
  const save = async () => {
    ...
    if (task && !dirty) {
      ...
      const saved = await api.workbenchImageSavePlans(task.id, task.revision, editedPlans)
      acceptSaved(saved)
      return saved
    }
    const saved = await api.workbenchImageSave(
      {
        ...brief,
  ...
  const openTask = (item: ImageTask) => {
    ...
    void navigation.open(item.id, async current => {
      const latest = await api.workbenchImageGet(item.id)
      if (!current()) return
  ...
  const act = async (action: ImageAction) => {
    ...
    try {
      const current = await api.workbenchImageAction(id, source.revision, { group, ...action }, config)
  ...
  const patch = (p: Partial<ImageBrief>) => {
    ...
    setBrief(next)
    // Preserve edits even if navigation unmounts this feature before bootstrap finishes.
    setDraftSaved(storeStudioDraft({ ...readStudioDraft(`feature:${feature}`), brief: next, modified: true,
      ...(task ? { taskId: task.id, revision: task.revision } : {}), plans: editedPlans, stage }))
  }
  ...
  dropReadyRef.current = {
    accept: routeActive && view !== 'tasks' && (view === 'workflow' || stage === 'brief'),
  }
  ...
  const importFromPaths = (paths: string[], asFolder: boolean) =>
    perform(async current => {
      ...
      const products = await api.workbenchImageImport(
        paths,
  ...
  const importWorkflowSources = (paths: string[]) =>
    perform(async current => {
      ...
      const products = await api.workbenchImageImport(paths, false)
      if (!current()) return
      editorVersion.current++
        ...
        <main className="is-main custom-scrollbar">
          ...
          {view === 'tasks' ? (
            <TaskPanel onDeleted={id => { setTasks(current => current.filter(t => t.id !== id)); if (task?.id === id) setTask(null) }} activeOperations={operations} tasks={tasks} loading={loading} currentId={task?.id} onOpen={openTask} library={library} disabled={false}
              onNew={() => switchView(feature, undefined, true)}
              onRefresh={async () => { const next = await api.workbenchImageBootstrap(); setTasks(next.tasks.filter(task => task.brief.feature === feature)); setTemplates(next.templates) }} />
          ) : view === 'workflow' ? (
            <ImageWorkflow
            ...
            <>
              ...
              {stage === 'brief' && (
                <Form configurationIssue={configurationIssue} model={config.model} protocol={config.protocol} brief={brief} templates={templates} busy={busy || loading}
                  ...
                  onImportExamples={() => void perform(async current => {
                    ...
                    if (!paths) return
                    const products = await api.workbenchImageImport(Array.isArray(paths) ? paths : [paths], false)
                    if (current()) mergeReplaceSources(products)
                  })}
      ...
      {settingsOpen && (
        <ConfigPanel
          ...
          onSave={async (c) => {
            ...
            localStorage.setItem('dsivio.workbench.image-project-settings', JSON.stringify(c))
            setConfig(c)
            ...
            <div className="is-lightbox-tools custom-scrollbar">
              ...
              <div className="is-actions">
                <Button
                  ...
                  onClick={() =>
                    void perform(async () => {
                      if (selected.path) await api.workbenchImageOpen(selected.path)
                    })
                  }
                >
                  <FolderOpen size={14} />
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void perform(async () => {
                        if (task) {
                          updateTemplate(
                            await api.workbenchImageFreeze(task.id, selected.productId, freezeName),
                          )
                          setSelected(null)
            ...
            <div className="is-dialog-actions">
              ...
              <Button
                ...
                onClick={() =>
                  void perform(async () => {
                    if (!task) return
                    const path = await api.workbenchImageExport(
                      task.id,
                      '',
                      delivery.width,
                      delivery.height,
