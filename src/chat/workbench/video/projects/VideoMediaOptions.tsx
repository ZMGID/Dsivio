...
export function VideoMediaOptions({
  ...
  const modes = videoModel(b.model || '')?.modes
  const supports = (value: 'text' | 'image' | 'frames' | 'reference') => !modes || modes.includes(value)
  const reference =
    mode === 'reference' ||
    ...
    <>
      ...
      >
        ...
        >
          ...
          {supports('text') && <option value="text">文生视频</option>}
          {supports('image') && <option value="image">单图驱动首帧</option>}
          {supports('reference') && <option value="reference">参考素材生成</option>}
          {supports('frames') && (
