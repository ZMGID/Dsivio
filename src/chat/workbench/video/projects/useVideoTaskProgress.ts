...
export function useVideoTaskProgress(tasks: VideoTask[], enabled: boolean, onUpdate: (task: VideoTask) => void) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      const running = current.current.tasks.filter(t => ['running', 'submitting'].includes(t.status))
      ...
      const results = await Promise.allSettled(batch.map(async t => {
        const latest = await api.workbenchVideoTask('get', { id: t.id })
        if (stopped) return
        const result = latest.status === 'running'
          ? await api.workbenchVideoTask('poll', { id: latest.id, revision: latest.revision }) : latest
