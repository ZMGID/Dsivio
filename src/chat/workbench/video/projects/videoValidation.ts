...
export function videoInputIssue(b: VideoBrief, analysis = b.mode === 'analysis', remake = false, script = ''): string {
  ...
  if (b.providerId && b.model) {
    ...
    return '' // Shared model catalog validates media roles and limits before submission.
  }
  if (!b.route || !videoRatios[b.route]) return '请选择生成服务'
  const route = b.route
  const resolutions = { grok: ['480p', '720p', '1080p'], minimax: ['768P', '2K'], comfy: ['0.5', '1'], native: [] as string[] }
  ...
  const low = { grok: 1, minimax: 4, comfy: 2, native: 1 }[route]
