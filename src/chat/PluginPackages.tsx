...
export function PluginPackages({ lang }: { lang: Lang }) {
  ...
  return <section className="mb-6 space-y-3 rounded-xl border border-neutral-200 p-4 dark:border-neutral-700" aria-label={zh ? '通用插件' : 'Plugin packages'}>
    ...
    <p className="text-xs text-neutral-500">{zh
      ? '导入 Kivio / Claude Code / Codex 格式的插件包，供内置 Dsivio Agent 使用。个人范围生效；外部 CLI 使用各自的插件配置。'
      : 'Import Kivio / Claude Code / Codex packages for the built-in Dsivio Agent. Packages apply across your projects; external CLIs use their own configuration.'}</p>
