...
export function SubAgentToolCard({ toolCall }: { toolCall: ToolCallRecord }) {
  ...
  const operations: Record<string, string> = { resolve: t('处理子代理结果', 'Resolve assignment'), list: t('查看子代理', 'List sub-agents'), get: t('读取子代理结果', 'Read sub-agent result'), wait: t('等待子代理', 'Wait for sub-agents'), message: t('补充消息', 'Send information'), continue: t('继续子代理', 'Continue sub-agent'), stop: t('停止子代理', 'Stop sub-agent') }
  const reasons: Record<string, string> = { needs_review: t('有结果等待主代理处理', 'Results need a parent decision'), timeout: t('等待超时，子代理继续运行', 'Wait timed out; children keep running'), result_ready: t('收到子代理结果', 'Results received'), all_finished: t('子代理均已结束', 'All children finished'), user_input: t('收到用户输入', 'User input received') }
  ...
  const prompt = string(args.message)
  ...
  const label = operation === 'message'
    ...
    : [waiting && !operationError && ['result_ready', 'needs_review', 'all_finished'].includes(waitReason)
      ? t('子代理结果', 'Sub-agent results')
      : operations[operation] || t('子代理操作', 'Sub-agent operation'), name].filter(Boolean).join(' · ')
  const status = operationError ? t('操作未完成', 'Operation failed')
  ...
  const reason = string(object(receipt.resolution).reason)
  const issue = toolCall.error || string(receipt.error) || (operationError ? t('操作未完成', 'Operation failed') : '')
  return <section className="not-prose my-1 min-w-0 text-[12.5px] leading-5">
    ...
    {expanded && <div className="ml-5 space-y-2 border-l border-neutral-200 py-2 pl-3 dark:border-neutral-700">
      ...
      {reason && <p className="whitespace-pre-wrap break-words text-neutral-500">{reason}</p>}
