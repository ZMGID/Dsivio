...
export function WorkflowHooksPanel({ lang }: { lang: Lang }) {
  ...
  return <section className="mb-5 space-y-3 rounded-xl border border-neutral-200 p-4 dark:border-neutral-700">
    ...
    <details className="text-xs text-neutral-500"><summary>{zh ? '配置示例与执行约定' : 'Configuration example and execution contract'}</summary>
      ...
      <p>{zh ? '脚本通过 stdin 接收 JSON。工具 matcher 使用 dsivio 工具名（如 bash、read、write、agent）；退出码 2 可拒绝提示或工具。用 args 数组可避免 Shell 转义问题。输出支持 hookSpecificOutput.additionalContext 与 PreToolUse.updatedInput；权限检查不会被跳过。' : 'Scripts receive JSON on stdin. Tool matchers use dsivio names (bash, read, write, agent). Exit code 2 blocks prompts or tools. An args array bypasses shell parsing. Output supports hookSpecificOutput.additionalContext and PreToolUse.updatedInput; host permissions still apply.'}</p>
    </details>
