...
describe('ComputerControlTab', () => {
  ...
  it('routes Ziniao installation to the official AI setup flow', async () => {
    const install = vi.fn().mockResolvedValue(undefined)
    render(<ComputerControlTab lang="zh" tools={defaultChatTools()} onChange={vi.fn()} onRequestAiInstall={install} />)
    const button = await screen.findByRole('button', { name: '让 AI 安装' })
    fireEvent.click(button)
    await waitFor(() => expect(install).toHaveBeenCalledWith('ziniao-cli'))
    expect(api.pluginsRunOfficialInstall).not.toHaveBeenCalled()
    expect(api.computerControlInstall).not.toHaveBeenCalled()
  })

  it('keeps installed Ziniao skills and enable control available', async () => {
    const status = pluginStatus({ id: 'ziniao-cli', name: '紫鸟 CLI', enabled: false, skillCount: 12, canInstall: false })
    vi.mocked(api.pluginsList).mockResolvedValue([status])
    vi.mocked(api.pluginsSetEnabled).mockResolvedValue({ ok: true, status: { ...status, enabled: true }, message: '' })
    render(<ComputerControlTab lang="zh" tools={defaultChatTools()} onChange={vi.fn()} />)
    const toggle = await screen.findByRole('switch', { name: '紫鸟 CLI 控制' })
    expect(screen.getByText(/12 Skill/)).toBeTruthy()
    fireEvent.click(toggle)
    await waitFor(() => expect(api.pluginsSetEnabled).toHaveBeenCalledWith('ziniao-cli', true))
  })
