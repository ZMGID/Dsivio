...
| 从零设计模板 | 先形成可复用 smart 模板，再按商品写提示词 | 新增「制作与试品」先制作共用规则，再用其他商品试做；旧「从零设计」入口保留原流程 |
| 样品选择 | 可选择有代表性的两个 SKU | 「制作与试品」可以选择 1–2 款；旧分类批量入口仍取分类前两款 |
| 模板修改后批量重出 | 可按 SKU 和槽位组合重出 | 新流程反馈/手动修改共用规则后，自动重做已选试品并撤回确认；尚无部分槽位批量重出 |
| 成套预览 | 提供整套拼图便于对照 | 新流程逐页并排对照原始参考与成图，保留历史版本；尚无独立整套拼图导出 |
...
制作—试品—反馈—持续出图的实现与状态规则见 [图片制作与试品流程](image-creation-workflow-2026-09-09.md)。
