...
pub fn native_studio_tool() -> ChatToolDefinition {
    ChatToolDefinition {
        ...
        description: format!("Use dsimage/dsvideo directly in conversation with the SAME tasks, models, templates and results as the image/video pages. Read the bundled skill studio guide first. bootstrap lists shared tasks/templates/config; get retrieves a task. Use current id/revision for mutations, never write task JSON directly. Image: import, save, save_plans, action, config, template_import, template_save. Video actions: {}. In chat, persist your own script with plan_result/analysis_result (input: id, revision, script), then approve the user-confirmed script and persist its converted prompt with prompt_result (input: id, revision, prompt), then quote/submit/poll. plan/analyze/prepare instead ask the host model to do that work; do not call them again when you already have the result. draft_get/draft_save share page drafts (image entry main; video creation/analysis/remake); draft_save requires current revision and value. Follow task approval gates. Honor existing confirmation for the unchanged script and generation terms; ask only for missing confirmation. submit requires confirmSpend:true. The host owns ComfyUI upload, submission and polling. On unknown actions, use the listed operations or report an interface mismatch and stop; do not search installation files or bypass this tool with scripts/MCP submission.", crate::video_studio::public_actions().join(", ")),
