...
fn validate(protocol: &str, model: &str, input: &VideoInput) -> Result<(), String> {
    ...
    if reference && !matches!(protocol, "minimax_h3" | "seedance") {
        return Err("此协议入口不支持多模态参考，请使用其专用接口".into());
    }
    if matches!(protocol, "minimax_h3" | "seedance") && reference {
        if input.first_frame.is_some() || input.last_frame.is_some() {
    ...
    {
        if !(media.starts_with("https://") || media.starts_with("http://")) {
...
async fn send(
    ...
    if !response.status().is_success() {
        return Err(format!(
            "视频接口 HTTP {}；请检查密钥、权限与请求参数",
            response.status().as_u16()
        ));
    }
    response.json().await.map_err(|_| {
        "视频接口返回了无效 JSON；若为提交请求，结果不确定，请核查供应商控制台，勿自动重试".into()
    })
}
