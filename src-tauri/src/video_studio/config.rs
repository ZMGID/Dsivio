...
pub(crate) fn current_comfy_server(
    server: &crate::settings::ChatMcpServer,
) -> Result<crate::settings::ChatMcpServer, String> {
    ...
    let provider: serde_json::Value = match std::fs::read(&config_path) {
        Ok(bytes) => {
            serde_json::from_slice(&bytes).map_err(|_| "视频配置文件格式无效，请检查视频设置")?
        }
...
fn comfy_endpoint(
    ...
    env.get("DSVIDEO_COMFY_TASK_URL")
        .map(String::as_str)
        .or_else(|| {
            provider["providers"]["comfy"]["base_url"]
                .as_str()
                .filter(|url| !url.trim().is_empty())
        })
        .unwrap_or("http://192.168.1.171:8188")
        .to_string()
}
...
mod tests {
    ...
    fn chat_uses_latest_provider_but_recovery_keeps_original_endpoint() {
        ...
        let provider =
            serde_json::json!({"providers":{"comfy":{"base_url":"http://192.168.1.171:8188"}}});
        assert_eq!(comfy_endpoint(&env, &provider), "http://192.168.1.171:8188");
        env.insert(
            "DSVIDEO_COMFY_TASK_URL".into(),
            "http://original:8188".into(),
        );
