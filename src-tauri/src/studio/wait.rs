    ...
    loop {
        ...
        tokio::select! {
            result = async { poll.as_mut().unwrap().await }, if poll.is_some() => {
                poll = None;
                next_poll = Instant::now() + crate::workbench::video_projects::POLL_INTERVAL;
...
pub async fn task(app: AppHandle, domain: &str, input: &Value) -> Result<Value, String> {
    ...
    wait_for(
        ...
        || {
            if domain == "image" {
                serde_json::to_value(crate::workbench::image_projects::workbench_image_get(id.into())?)
                    .map_err(|e| e.to_string())
            } else {
                crate::workbench::video_projects::task_snapshot(id)
            }
        },
        || {
            let app = app.clone();
            ...
            tokio::spawn(async move { crate::workbench::video_projects::poll_task(app, &id).await })
        },
    )
    .await
}
