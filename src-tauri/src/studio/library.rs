...
#[tauri::command]
pub async fn studio_task_file_action(
    domain: String,
    id: String,
    action: String,
) -> Result<(), String> {
    ...
    let target = tauri::async_runtime::spawn_blocking(move || {
        task_file_action_at(&root, &domain, &id, &action)
    })
    .await
...
fn task_file_action_at(
    ...
    if !matches!(action, "reveal" | "delete") {
    ...
    let task: serde_json::Value =
        serde_json::from_slice(&std::fs::read(&path).map_err(|_| "任务不存在，请刷新列表")?)
            .map_err(|_| "任务记录无法读取")?;
    if action == "reveal" {
        // Completed videos reveal their output; drafts reveal their saved task record.
        if domain == "video" {
            if let Some(output) = task["output"].as_str() {
                if let (Ok(output), Ok(base)) = (
                    std::path::PathBuf::from(output).canonicalize(),
                    directory.canonicalize(),
                ) {
                    if output.starts_with(base) && output.is_file() {
...
fn managed_references(
    ...
    match value {
        ...
        serde_json::Value::Array(values) => {
            for value in values {
                managed_references(value, directory, out);
            }
        }
        ...
        for folder in ["tasks", "templates", "drafts"] {
            ...
            while let Some(folder) = pending.pop() {
                for entry in std::fs::read_dir(folder).map_err(|e| e.to_string())? {
                    ...
                    if path == target || path.extension().is_none_or(|ext| ext != "json") {
                        continue;
                    }
                    ...
                    if let Some(output) = value["outputDirectory"].as_str() {
                        if let Ok(path) = Path::new(output).canonicalize() {
...
fn remove_owned(
    path: &Path,
    shared: &std::collections::BTreeSet<std::path::PathBuf>,
) -> Result<(), String> {
    if !path.exists() {
        return Ok(());
    }
    ...
    if shared
        .iter()
        .any(|reference| canonical == *reference || canonical.starts_with(reference))
    {
        return Ok(());
    }
    if metadata.is_dir() {
...
fn delete_task_files(
    ...
    candidates.retain(|path| {
        path.is_file()
            && ["assets", "results", "outputs", "exports"]
                .iter()
                .any(|folder| path.starts_with(base.join(folder)))
    });
    ...
    if let Some(output) = task["outputDirectory"].as_str() {
        ...
        if output.exists() {
            if !output
                .file_name()
                .is_some_and(|name| name.to_string_lossy().ends_with(&format!("__{id}")))
    ...
    let mut organization: BTreeMap<String, Organization> = if organization_path.exists() {
        serde_json::from_slice(&std::fs::read(&organization_path).map_err(|e| e.to_string())?)
            .map_err(|e| e.to_string())?
    } else {
        BTreeMap::new()
    };
    for path in candidates {
        remove_owned(&path, &shared)?;
    }
...
fn library_at(
    ...
    if ids.is_empty() || ids.len() > 200 {
        return Err("每次请选择 1–200 个任务".into());
    }
...
mod tests {
    ...
    fn task(root: &Path, domain: &str, id: &str, status: &str) {
        ...
        std::fs::write(
            ...
            serde_json::json!({"id":id,"status":status,"revision":7}).to_string(),
        )
        .unwrap();
    }
    #[test]
    fn deletion_preserves_assets_referenced_by_shared_drafts_and_nested_templates() {
        ...
        std::fs::write(
            base.join("drafts/main.json"),
            r#"{"revision":1,"value":{"brief":{"images":["assets/draft.png"]}}}"#,
        )
        .unwrap();
        std::fs::create_dir_all(base.join("templates/set")).unwrap();
        std::fs::write(
            base.join("templates/set/template.json"),
            r#"{"image":"assets/template.png"}"#,
        )
        .unwrap();
    ...
    fn deletion_rejects_a_video_task_locked_by_a_worker() {
        ...
        lock.lock().unwrap();
        assert!(task_file_action_at(root.path(), "video", &id, "delete")
            .unwrap_err()
            .contains("任务正在处理"));
        assert!(record.exists());
        drop(lock);
    ...
    fn file_actions_reveal_drafts_and_delete_only_idle_records() {
        ...
        assert_eq!(
            task_file_action_at(root.path(), "video", &id, "reveal").unwrap(),
            Some(path.clone())
        );
    ...
    fn deletes_outputs_and_exclusive_assets_but_keeps_shared_and_original_files() {
        ...
        std::fs::write(
            base.join(format!("tasks/{other}.json")),
            serde_json::json!({"id":other,"status":"draft","brief":{"images":[shared]}})
                .to_string(),
        )
        .unwrap();
        task_file_action_at(root.path(), "video", &id, "delete").unwrap();
    ...
    fn deletes_custom_image_output_folder_and_preserves_template_assets() {
        ...
        std::fs::write(
            base.join("templates/test.json"),
            r#"{"image":"assets/shared.png"}"#,
        )
        .unwrap();
        task(root.path(), "image", &id, "ready");
    ...
    fn organization_survives_reload_without_changing_generation_and_isolates_domains() {
        ...
        assert!(library_at(root.path(), "image", None, None)
            .unwrap()
            .is_empty());
        let saved: serde_json::Value = serde_json::from_slice(
            &std::fs::read(root.path().join(format!("video-studio/tasks/{id}.json"))).unwrap(),
        )
        .unwrap();
        assert_eq!(saved["revision"], 7);
        assert_eq!(saved["status"], "succeeded");
        let restored = library_at(
            root.path(),
            "video",
            Some(vec![id.clone()]),
            Some(OrganizationPatch {
    ...
    fn invalid_batch_does_not_partially_archive_or_escape_the_workspace() {
        ...
        for ids in [
            vec![ready.clone(), running],
            vec![ready, "../outside".into()],
        ] {
            assert!(library_at(
                root.path(),
                "image",
