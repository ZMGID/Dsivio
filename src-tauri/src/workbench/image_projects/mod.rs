...
pub(crate) mod agent;
#[cfg(test)]
use crate::content_templates::builtins;
mod media;
mod generation;
mod imports;
mod output;
...
#[tauri::command]
pub fn workbench_image_bootstrap(app: AppHandle) -> Result<Value, String> {
    let _lock = lock()?;
...
#[tauri::command]
pub fn workbench_image_get(id: String) -> Result<Task, String> {
    let _lock = lock()?;
...
pub fn workbench_image_save(
    ...
    output::prepare(&mut t, &config.unwrap_or(storage::config()?))?;
...
#[tauri::command]
pub fn workbench_image_save_plans(
    id: String,
    revision: u64,
    plans: Vec<ImagePlan>,
) -> Result<Task, String> {
...
#[tauri::command]
pub async fn workbench_image_import(
    paths: Vec<String>,
    as_products: bool,
...
#[tauri::command]
pub async fn workbench_image_preview(path: String, original: Option<bool>) -> Result<String, String> {
...
pub fn workbench_image_action(
    ...
    let cfg = config.ok_or("请选择本次使用的工作台图片模型")?;
    ...
    if matches!(
        ...
        media::validate(&cfg, &t.brief)?;
        media::provider(&app, &cfg)?;
    }
...
async fn resume(
    ...
    if let Some(id) = remote.strip_prefix("media:") {
        crate::media_generation::get_media_task(app.clone(), id.into(), Some(true))?;
    }
    ...
    media::store_image(&t.id, &mut t.results[index], &bytes)
}

#[tauri::command]
pub fn workbench_image_freeze(
    id: String,
    product_id: String,
    name: String,
...
#[tauri::command]
pub async fn workbench_image_export(
    id: String,
    destination: String,
    width: u32,
    height: u32,
...
#[tauri::command]
pub async fn workbench_image_open(path: Option<String>) -> Result<(), String> {
