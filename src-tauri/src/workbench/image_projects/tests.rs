...
fn originals_and_deliveries_are_grouped_and_never_overwrite_previous_versions() {
    ...
    media::store_image_in(&task, &mut a, bytes.get_ref()).unwrap();
    media::store_image_in(&task, &mut b, bytes.get_ref()).unwrap();
    assert_ne!(
        output::original_relative(&task, &a, "png").parent(),
        output::original_relative(&task, &b, "png").parent()
    );
    assert!(media::store_image_in(&task, &mut a, bytes.get_ref()).is_err());
    let old = a.clone();
    let mut revised = result("a", task.revision, None);
    media::store_image_in(&task, &mut revised, bytes.get_ref()).unwrap();
...
impl generation::Backend for GenerationBackend {
    fn submit<'a>(
        &'a self,
        plan: &'a ImagePlan,
    ) -> futures::future::BoxFuture<'a, Result<media::Submission, String>> {
        Box::pin(async move {
            assert!(
                self.snapshots
                    .lock()
            ...
            if self.download_receipt {
                return Ok(media::Submission::Download(format!(
                    "https://cdn.example/{}",
                    plan.slot_id
                )));
            }
            if self.remote {
                return Ok(media::Submission::Pending(plan.slot_id.clone()));
            }
            ...
            Ok(media::Submission::Image(plan.slot_id.as_bytes().to_vec()))
        })
    }
...
fn auto_edits_route_logo_to_each_target_and_preserve_delivery_dimensions() {
    ...
    assert_eq!(
        media::resolved_gen_brief(&cfg, &task.brief, &plans[1])
            .unwrap()
            .ratio,
        "16:9"
    );
    ...
    assert_eq!(
        media::resolved_gen_brief(&cfg, &manual, &plans[1])
            .unwrap()
            .ratio,
        "1:1"
    );
    ...
    media::store_image_in(&task, &mut saved, generated.get_ref()).unwrap();
