...
use super::{agent, media, generation, persist, stopped, storage, types::*, workflow};
...
async fn derive(
    ...
    let mut result = if let Some(prior) = prior {
        ...
        let submission = media::submit(app, cfg, &task.id, &task.brief, &plan).await;
        let mut result = result;
        match submission {
            Ok(media::Submission::Image(bytes)) => {
                if let Err(error) = store_derived(&mut result, &bytes) {
                    result.error = Some(error);
                }
            }
            Ok(media::Submission::Pending(id)) => result.remote_id = Some(id),
            Ok(media::Submission::Download(url)) => result.download_url = Some(url),
