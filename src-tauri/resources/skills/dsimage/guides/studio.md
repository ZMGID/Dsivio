...
原脚本的批次格式仍是 `_dsimage/batch.json`，原图和交付文件留在用户指定的成图目录。页面任务使用 `tasks/*.json`；`studio tasks` 可以查到页面任务文件及其素材、方案、结果的入口，素材相对路径以 `studio paths` 的 workspace 为根。不要把两种任务 JSON 当成同一种格式直接覆盖；从已有结果复用模板，使用标准模板目录交换，或在原任务入口继续处理。
