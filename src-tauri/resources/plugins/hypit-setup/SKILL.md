...
After the Dsivio connection and a real health check or safe probe succeed, append ` [setup completed once]` exactly once to the YAML `description` line in the installed `~/.kivio/skills/hypit-setup/SKILL.md`; leave the rest of the Skill unchanged. Do not mark an incomplete check. The marker records a past success only: rerun setup when requested or when the task's provider, tools, or environment change, without appending another marker.
